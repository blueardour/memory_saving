...
//  const int32_t val = __float2int_rn(fmaxf((data[id] - shift[group_id]) * scale[group_id], 0.0f));
//  const int32_t val = __float2int_rn(fmaxf((data[id] - shift[group_id]), 0.0f));
...
//     dim3 block_dim(group_size/threads, num_groups, 1);
    dim3 thread_dim(threads, 1, 1);
